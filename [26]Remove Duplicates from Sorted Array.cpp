...
// 
//
// Related Topics Array Two Pointers 👍 16369 👎 19628
...
public:
    int removeDuplicates(vector<int>& nums) {
      int base = 0; //First Occurance of that number
      int writeOver = 1; //Were writing over in the list
      for (int i = 1; i < nums.size(); i++) {
          if (nums[i] != nums[base]) {
            nums[writeOver] = nums[i];
            base = i;
            writeOver++;
          }
        }
